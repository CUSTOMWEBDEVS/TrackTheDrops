(function(){
  ...
  const getTune=()=>PROFILES['aggressive'];
  ...
  function filterBlobs(binary,w,h,tune,edges){
    const visited=new Uint8Array(w*h),keep=new Uint8Array(w*h),mag=edges.mag,ori=edges.ori;
    const minArea=Math.max(6,Math.floor(w*h*tune.minAreaFrac)),maxArea=Math.floor(w*h*tune.maxFrac);
    function flood(start){
      ...
      const ok=(area>=minArea)&&(area<=maxArea)&&(solidity>=tune.minSolidity)&&(oriDominance<=tune.maxOriDominance);
  ...
  async function toggleFS(){
    ...
    if(isFs){
      ...
      // keep CSS fullscreen too: toggle class
  ...
  function loop(ts){
    try{
      ...
      const d=color.data, sens=1.0, thr=0.0; // fixed to your preferred values
      ...
      const need=1;
      const {keep,blobs}=filterBlobs(bin,procW,procH,tune,edgesCache);
