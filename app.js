(function(){
  ...
  const BASE={vMaxBase:0.55,vMin:0.10,sMinBase:0.60,hTolBase:12,aMinBase:40,bMaxBase:18,aDivBRatio:2.1,crRelBase:100,yMaxBase:175,
    minArea:160,maxFrac:0.12,edgeMagThresh:70,maxEdgeDensity:0.20,maxSpecDensity:0.015,minSolidity:0.82,maxOriDominance:0.48};
  const PROFILES={safety:{...BASE},balanced:{...BASE,vMaxBase:0.60,sMinBase:0.58,aMinBase:38,bMaxBase:20,crRelBase:95,yMaxBase:182,minArea:140,maxFrac:0.14},
    aggressive:{...BASE,vMaxBase:0.68,sMinBase:0.52,aMinBase:32,bMaxBase:25,crRelBase:85,yMaxBase:190,minArea:110,maxFrac:0.18,edgeMagThresh:60,maxEdgeDensity:0.24,maxSpecDensity:0.03,minSolidity:0.78,maxOriDominance:0.55}};
  ...
  function scorePixel(r,g,b,sens,tune){
    ...
    const r_g=r/Math.max(1,g),r_b=r/Math.max(1,b);if(r_g<(1.3-0.1*sens)||r_b<(2.0-0.3*sens)||(r-g)<14||(r-b)<28)return 0;
    const {y,cb,cr}=toYCbCr(r,g,b);const crRel=cr-0.55*cb;if(y>tune.yMaxBase+25*(1-sens)||crRel<tune.crRelBase-20*(1-sens))return 0;
    const {L,a,b:bb}=toLab(r,g,b);const aMin=tune.aMinBase-8*(1-sens);const bMax=tune.bMaxBase+8*(1-sens);if(a<aMin||bb>bMax||(a/Math.max(1e-3,bb))<tune.aDivBRatio||L>62)return 0;
    const hueScore=Math.max(0,1-hueDistDeg(hDeg,0)/(hTol+1));const satScore=Math.min(1,(s-0.35)/0.5);const valScore=Math.min(1,(0.72-v)/0.4);
    const crCbScore=Math.min(1,(crRel-85)/85);const labScore=Math.min(1,((a-aMin)/30+(Math.max(0,bMax-bb)/20))/2);const ratioScore=Math.min(1,((r_b-1.6)/1.5+(r_g-1.15)/0.7)/2);
  ...
  async function start(){
    if(stream)return;
    try{el.status.textContent='Requesting camera…';stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}},audio:false})}
    ...
    el.startBtn.disabled=true;el.stopBtn.disabled=false;el.snapBtn.disabled=false;setMsg('Streaming…')
    ...
    function frame(ts){
      ...
      ctx.drawImage(el.video,0,0,W,H);const img=ctx.getImageData(0,0,W,H);
      for(let p=0;p<img.data.length;p+=4){const r=img.data[p],g=img.data[p+1],b=img.data[p+2];const y=(r*0.2126+g*0.7152+b*0.0722)|0;img.data[p]=img.data[p+1]=img.data[p+2]=y}
      ctx.putImageData(img,0,0);

      const d=img.data,sens=Number(el.sens.value)/100,thr=Number(el.thr.value)/100,tune=getTune();
      ...
      const kept=filterBlobs(bin,W,H,img,tune);
  ...
  el.helpBtn.onclick=()=>alert('This is not thermal. Debug score shows blood-likeness by color. Use Safety profile to reduce leaves/wood/plastics. Increase Threshold/Stability in bright scenes.');
