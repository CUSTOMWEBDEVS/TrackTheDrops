(function(){
  ...
  el.openSettings && el.openSettings.addEventListener('click', toggleDrawer);
  el.closeDrawer && el.closeDrawer.addEventListener('click', toggleDrawer);
  ...
  // Tap anywhere to toggle UI if already streaming; **no auto-start on tap**
  el.viewport.addEventListener('click',()=>{
    if(!stream){ showUI(); return; } // was: start(); return;
    if(el.app.classList.contains('hiddenUI')) showUI(); else el.app.classList.add('hiddenUI');
  });
  // If you still want tap-to-start, only the hint starts it:
  el.tapHint && el.tapHint.addEventListener('click',()=>{ if(!stream) start(); });

  // ====== Ranges display ======
  const bindRange=(r,lab,fmt=v=>Number(v/100).toFixed(2))=>{if(!r||!lab)return; const f=()=>lab.textContent=fmt(r.value);r.addEventListener('input',f);f()}
  ...
  async function hapticPulse(){
    if(hasCap){
      try{ await window.Capacitor.Haptics.impact({ style: 'medium' }); return; }catch{}
    }
  ...
  function toYCbCr(r,g,b){const y=0.299*r+0.587*g+0.114*b;const cb=128-0.168736*r-0.331264*g+0.5*b;const cr=128+0.5*r-0.418688*g+0.081312*-1*b;return {y,cb,cr}}
  ...
  el.fsBtn && el.fsBtn.addEventListener('click', toggleFS);
  el.exitFS && el.exitFS.addEventListener('click', toggleFS);

  let deferredPrompt=null;
  window.addEventListener('beforeinstallprompt',e=>{ e.preventDefault(); deferredPrompt=e; if(el.installBtn) el.installBtn.style.display='inline-flex'; showUI(); });
  el.installBtn && el.installBtn.addEventListener('click', async()=>{ if(deferredPrompt){ await deferredPrompt.prompt(); deferredPrompt=null } });

  el.alertBtn && el.alertBtn.addEventListener('click',()=>{
  ...
  const doAlert=()=> el.alertBtn && el.alertBtn.getAttribute('aria-pressed')==='true';
  ...
  function loop(ts){
    ...
    let anyHit=false;
    // Draw **purple** double-ring circles for each blob that has enough stable pixels
  ...
  // Detection math (dupe-safe helpers kept in this IIFE)
  function srgb2lin(c){c/=255;return(c<=0.04045)?c/12.92:Math.pow((c+0.055)/1.055,2.4)}
  function toLab(r,g,b){const R=srgb2lin(r),G=srgb2lin(g),B=srgb2lin(b);const X=0.4124564*R+0.3575761*G+0.1804375*B;const Y=0.2126729*R+0.7151522*G+0.0721750*B;const Z=0.0193339*R+0.1191920*G+0.9503041*B;const Xn=0.95047,Yn=1.0,Zn=1.08883;const f=t=>{const d=6/29;return(t>Math.pow(d,3))?Math.cbrt(t):t/(3*d*d)+4/29};const fx=f(X/Xn),fy=f(Y/Yn),fz=f(Z/Zn);return {L:116*fy-16,a:500*(fx-fy),b:200*(fy-fz)}}
  function toHSV(r,g,b){const rn=r/255,gn=g/255,bn=b/255;const max=Math.max(rn,gn,bn),min=Math.min(rn,gn,bn),d=max-min;let h=0;if(d!==0){if(max===rn)h=((gn-bn)/d+(gn<bn?6:0));else if(max===gn)h=((bn-rn)/d+2);else h=((rn-gn)/d+4);h/=6}const s=max===0?0:d/max,v=max;return {h,s,v}}
  ...
  // Hook up buttons
  el.startBtn && el.startBtn.addEventListener('click',()=>{ stream?stop():start() });
  el.torchBtn && el.torchBtn.addEventListener('click', toggleTorch);
