(function(){
  ...
  const el={app:$('app'),viewport:$('viewport'),video:$('video'),overlay:$('overlay'),tapHint:$('tapHint'),
    status:$('status'),fps:$('fps'),startBtn:$('startBtn'),torchBtn:$('torchBtn'),alertBtn:$('alertBtn'),flash:$('flash'),
    sens:$('sens'),thr:$('thr'),stability:$('stability'),opacity:$('opacity')};
  ...
  // ---------- Color helpers ----------
  function toHSV(r,g,b){const rn=r/255,gn=g/255,bn=b/255;const max=Math.max(rn,gn,bn),min=Math.min(rn,gn,bn),d=max-min;let h=0;if(d){if(max===rn)h=((gn-bn)/d+(gn<bn?6:0));else if(max===gn)h=((bn-rn)/d+2);else h=((rn-gn)/d+4);h/=6}const s=max===0?0:d/max,v=max;return {h,s,v}}
  ...
  function blobs(binary,w,h,minArea,maxArea){
    ...
    function flood(s){
      ...
      while(q.length){ const i=q.pop(); area++; const x=i%w,y=(i-x)/w; cx+=x; cy+=y;
        const ns=[i-1,i+1,i-w,i+w,i-w-1,i-w+1,i+w-1,i+w+1];
        for(const n of ns){ if(n<0||n>=w*h) continue; if(!vis[n] && binary[n]){ vis[n]=1; q.push(n);} }
      }
      return {area, centroid:{x:cx/area,y:cy/area}};
    }
    for(let i=0;i<w*h;i++){ if(!vis[i] && binary[i]){ const r=flood(i); if(r.area>=minArea && r.area<=maxArea) out.push(r); } }
    return out;
  }
  ...
  async function start(){
    ...
    try{
      ...
      ]; let s=null; for(const c of tries){ try{ s=await navigator.mediaDevices.getUserMedia(c); if(s)break; }catch{} }
    ...
    }catch{ setStatus('Camera error'); } finally{ starting=false; }
  }
  function stop(){
  ...
  async function torch(){
    try{
      if(stream){ const t=stream.getVideoTracks()[0], caps=t.getCapabilities?.(); if(caps && 'torch' in caps){ const cons=t.getConstraints?.()||{}; const cur=!!(cons.advanced||[]).find(o=>o&&o.torch===true); await t.applyConstraints({advanced:[{torch:!cur}]}); return; } }
    }catch{} if(el.flash) el.flash.style.opacity=(el.flash.style.opacity==='1'?'0':'1');
  }
  el.torchBtn?.addEventListener('click',e=>{ e.stopPropagation(); torch(); },true);
  el.alertBtn?.addEventListener('click',e=>{ e.stopPropagation(); const on=el.alertBtn.getAttribute('aria-pressed')!=='true'; el.alertBtn.setAttribute('aria-pressed',String(on)); el.alertBtn.textContent=on?'Alert: On':'Alert: Off'; },true);
  ...
  function loop(ts){
    ...
    let max=-1e9,min=1e9;
    for(let p=0,i=0;p<d.length;p+=4,i++){ const s=isBloodish(d[p],d[p+1],d[p+2],sens)?1:0; score[i]=s; if(s>max)max=s; if(s<min)min=s; }
    ...
    if(alpha>0){
      ...
      for(let i=0,p=0;i<stable.length;i++,p+=4){ if(stable[i]){ md[p]=235; md[p+1]=20; md[p+2]=20; md[p+3]=Math.floor(alpha*255); } }
      pctx.putImageData(mask,0,0); octx.drawImage(proc,0,0,el.overlay.width,el.overlay.height);
    }
    let any=false;
