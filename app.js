(function(){
  ...
  const el={video:$('video'),overlay:$('overlay'),tapHint:$('tapHint'),status:$('status'),fps:$('fps'),
    ...
    viewport:$('viewport'), app:$('app'), drawer:$('drawer'), closeDrawer:$('closeDrawer') };
  ...
  const toggleDrawer=()=>{ el.drawer.classList.toggle('open'); el.drawer.setAttribute('aria-hidden', String(!el.drawer.classList.contains('open'))); showUI() }
  ...
  // ====== Ranges display ======
  const bindRange=(r,lab,fmt=v=>Number(v/100).toFixed(2))=>{ if(!r) return; const f=()=>lab.textContent=fmt(r.value); r.addEventListener('input',f); f() }
  ...
  async function hapticPulse(){
    ...
    try{
      ...
      if(aCtx){ const o=aCtx.createOscillator(), g=aCtx.createGain(); o.type='square'; o.frequency.value=1100; g.gain.value=0.05; o.connect(g); g.connect(aCtx.destination); o.start(); setTimeout(()=>o.stop(),60); }
    }catch{}
    flashOnce();
  }
  ...
  function filterBlobs(binary,w,h,img,tune,edges){
    ...
    const minArea=Math.max(6,Math.floor(w*h*getTune().minAreaFrac)),maxArea=Math.floor(w*h*getTune().maxFrac);
  ...
  async function start(){
    ...
    try{
      ...
      let s=null;
      for(const c of tries){ try{ s=await navigator.mediaDevices.getUserMedia({...c,audio:false}); if(s) break; }catch(e){ el.status.textContent=e.name; } }
      if(!s){ el.status.textContent='Camera failed'; starting=false; return; }
      stream=s; el.video.srcObject=stream; try{ await el.video.play() }catch{}
  ...
  function stop(){ if(anim) cancelAnimationFrame(anim); anim=null; if(stream){ stream.getTracks().forEach(t=>t.stop()); stream=null } el.tapHint.style.display=''; el.status.textContent='Stopped.'; showUI(); }
  ...
  function toggleFS(){ const on=!el.viewport.classList.contains('fs'); if(on){el.viewport.classList.add('fs')}else{el.viewport.classList.remove('fs')} setTimeout(()=>{ if(stream) sizeProcessingCanvas() },80); showUI(); }
  ...
  function loop(ts){
    ...
    const d=color.data, sens=Number(el.sens.value||100)/100, thr=Number(el.thr.value||0)/100, tune=(PROFILES[el.profile.value||'aggressive']||PROFILES.aggressive);
    const doLab=(!el.fastMode || !el.fastMode.checked)||(frameCount%2===0);
    ...
    const need=Number(el.stability?.value||1);
    ...
    const out=procCtx.createImageData(procW,procH), od=out.data, alpha=Number(el.opacity?.value||87)/100;
